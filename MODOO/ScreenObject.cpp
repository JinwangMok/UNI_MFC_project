...
void ScreenObject::SetRectsPosition(CPoint window_pt) {
	...
	if (this->screenType == SCR_MENU) {
		...
		if (this->screenType == SCR_PAYMENT_1 || this->screenType == SCR_PAYMENT_2) {
			for (int i = 0; i < 3; i++) {
				this-> rects[6+i] = CRect(
			...
			if (this->screenType == SCR_SET_DESSERT || this->screenType == SCR_SET_DRINK) {
				...
				if (this->screenType == SCR_CHICKEN) {
					for (int i = 0; i < 6; i++) {
